...
void TM0_IRQHandler(void)
{
  ...
  if((cfg.runMode == FFT) && cfg.rawPlot) {
    ssiof1_putcByte(0x50 + (((dotMax << 1) & 0xff00) >> 8));
    ssiof1_putcByte((dotMax << 1) & 0xff);
    set_bit(SSIOF1->SF1CTRL, 1);
    dmac1_Transfer(&video[0], (void*)&SSIOF1->SF1DWR, dotMax << 1);
  }
  else {
    ssiof1_putcByte(0x50 + ((dotMax & 0xff00) >> 8));
    ssiof1_putcByte(dotMax & 0xff);
    set_bit(SSIOF1->SF1CTRL, 1);
    dmac1_Transfer(&video[0], (void*)&SSIOF1->SF1DWR, dotMax);
  }
  dmac1_enable();
  //DACPORT = 255;
  VSYNCPORT |= VSYNC;
